...
const SCHEMA_EMITTER_WARNING_TYPE = 'ApiVersioningSchemaEmitterWarning';
...
function emitObject(
  ...
  const body = entries
    .map(([key, fieldSchema]) => {
      return indent(
        `${formatKey(key)}: ${emitSchema(fieldSchema, depth + 1, warn)},`,
        depth + 1,
      );
    })
    .join('\n');
...
function emitSchema(
  ...
  if (schema instanceof z.ZodOptional) {
    return `${emitSchema(schema.unwrap() as z.ZodTypeAny, depth, warn)}.optional()`;
  }

  if (schema instanceof z.ZodNullable) {
    return `${emitSchema(schema.unwrap() as z.ZodTypeAny, depth, warn)}.nullable()`;
  }
  ...
  if (schema instanceof z.ZodArray) {
    return `z.array(${emitSchema(schema.element as z.ZodTypeAny, depth, warn)})`;
  }

  if (schema instanceof z.ZodObject) {
    return emitObject(schema as z.ZodObject<z.ZodRawShape>, depth, warn);
  }

  if (schema instanceof z.ZodUnion) {
    const options = schema.options
      .map((option) => emitSchema(option as z.ZodTypeAny, depth + 1, warn))
      .join(', ');

    return `z.union([${options}])`;
  }

  const schemaTypeName = schema.constructor?.name ?? 'UnknownSchemaType';
  warn(
    `Unsupported Zod schema node "${schemaTypeName}" while emitting schema module. Falling back to z.any().`,
  );
  return 'z.any()';
}
...
export function emitZodSchemaModule(
  ...
  const defaultWarningHandler: WarningHandler = (message) => {
    process.emitWarning(message, { type: SCHEMA_EMITTER_WARNING_TYPE });
  };
  const warningHandler = options.onWarning ?? defaultWarningHandler;
  const warn: WarningHandler = (message) => {
  ...
  const expression = emitSchema(schema, 0, warn);
