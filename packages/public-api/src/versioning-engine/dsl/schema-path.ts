...
type ObjectPathSuffix<T> = NormalizeNode<T> extends Primitive
  ...
  ? {
      [K in KnownStringKeys<NormalizeNode<T>>]:
        NormalizeNode<NormalizeNode<T>[K]> extends Primitive
          ? never
 ...
 */
export type SchemaPathStrict<T> = PathSuffix<T>;
...
export type SchemaPath<T> = [SchemaPathStrict<T>] extends [never]
  ? string
  : SchemaPathStrict<T>;

export type SchemaObjectPathStrict<T> = ObjectPathSuffix<T>;

export type SchemaObjectPath<T> = [SchemaObjectPathStrict<T>] extends [never]
  ? string
...
export type SchemaFieldAtPath<
  T,
  TPath extends SchemaObjectPath<T>,
> = [KnownStringKeys<NormalizeNode<SchemaPathValue<T, TPath>>>] extends [never]
  ? string
