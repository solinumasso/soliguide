...
export type SearchAdministrativeStatus =
  | 'regular'
  | 'asylum'
  | 'refugee'
  | 'undocumented';

export type SearchFamilySituation =
  | 'isolated'
  | 'family'
  | 'couple'
  | 'pregnant';

export type SearchGender = 'men' | 'women';

export type SearchAudienceCharacteristic =
  | 'violence'
  | 'addiction'
  | 'disability'
  | 'lgbtqPlus'
...
export interface SearchQuery {
  ...
  department?: string;
  cityType?: 'city' | 'postalCode';
  cityValue?: string;
  latitude?: number;
  longitude?: number;
  radiusKm?: number;
  openToday?: boolean;
  accessKind?: 'unconditional' | 'conditional';
  accessModes?: SearchAccessMode[];
  audienceAdmissionPolicy?: 'open' | 'restricted';
  audienceIsTargeted?: boolean;
  audienceAdministrativeStatuses?: SearchAdministrativeStatus[];
  audienceMinAge?: number;
  audienceMaxAge?: number;
  audienceFamilySituations?: SearchFamilySituation[];
  audienceGenders?: SearchGender[];
  audienceOtherCharacteristics?: SearchAudienceCharacteristic[];
  updatedOn?: string;
