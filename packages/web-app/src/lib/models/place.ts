...
//! TODO Type tempInfos here after rewriting the BasePlaceTempInfos types
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export const computeTempInfo = (tempInfos: any): SearchResultTempInfo => {
  return (['hours', 'closure', 'message'] as const).reduce((acc, key) => {
    return {
      ...acc,
      [key]: new BasePlaceTempInfos(tempInfos[key]).status
    };
