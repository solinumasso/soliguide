...
<script lang="ts">
  ...
  export let closureDisplayMode: DisplayMode = 'regular';
...
<PlaceDetailsSection>
  <section class="opening-hours" id="openingHoursSection">
    ...
    <TempInfoBanner tempInfo={tempInfo.closure} tempInfoType={TempInfoType.closure}
    ></TempInfoBanner>
    <TempInfoBanner tempInfo={tempInfo.hours} tempInfoType={TempInfoType.hours}></TempInfoBanner>
    <ul>
          ...
          <span class="hours-container">
            <Text type="caption1Medium" color="neutral">
              {#if status === PlaceOpeningStatus.UNKNOWN}
                {$i18n.t('TIME_UNKNOWN')}
              {:else if closureDisplayMode === 'temporary'}
