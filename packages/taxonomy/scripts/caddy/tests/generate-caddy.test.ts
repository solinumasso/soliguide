...
describe("generate-caddy script", () => {
  it("generates redirections for historical slugs", async () => {
    ...
    // Entries only — map wrapper is now in the Caddyfile
    expect(content).not.toContain("map {http.request.orig_uri.path}");
    expect(content).toMatch(/\t~\^.*\$/);

    // Verify that old categories are redirected to new ones
    expect(content).toContain(
      // deepsource-disable-next-line JS-0038
      '~^(.*/search/.*)/(sante)(/.*)?$ "${2}" "${1}/health${3}"',
    );
    expect(content).toContain(
      // deepsource-disable-next-line JS-0038
      '~^(.*/search/.*)/(depistage)(/.*)?$ "${2}" "${1}/std_testing${3}"',
    );
