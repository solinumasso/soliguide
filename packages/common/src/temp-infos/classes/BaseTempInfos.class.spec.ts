 ...
 */
import { BasePlaceTempInfos } from "..";
import { addDays, subDays } from "date-fns";

// Create mock for formatInTimeZone
const mockFormatInTimeZone = jest.fn((date) => {
  // Simply return the ISO date for tests
...
// Mock for OpeningHours
const mockOpeningHours = jest.fn().mockImplementation(() => ({
  // Minimal mock for OpeningHours
}));
...
describe("BasePlaceTempInfos", () => {
  ...
  describe("infoColor property", () => {
    test("should be null when no dates are provided", () => {
      const tempInfo = new BasePlaceTempInfos();
    ...
    test("should handle real-world data example correctly", () => {
      ...
      const tempInfo = new BasePlaceTempInfos(data.closure);
